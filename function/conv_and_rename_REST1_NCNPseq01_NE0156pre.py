...
def Rename():
    ...
    for i in range(len(sublist)):
        ...
        for j in datelist_int:
            ...
            for fname in flist:
                if 'T1_MPR_20230428095322_26' in fname:
                    shutil.copy2(fname, T1_fpath)
                elif 'T2_SPC_20230428095322_25' in fname:
                    shutil.copy2(fname, T2_fpath)
                elif 'DWI_AP' in fname and '15.nii.gz' in fname:
                    shutil.copy2(fname, Diff_AP_SBRef_fpath)
                elif 'DWI_AP' in fname and '16.nii.gz' in fname:
                    shutil.copy2(fname, Diff_AP_fpath)
                elif '16.bval' in fname:
                    shutil.copy2(fname, Diff_AP_bval_fpath)
                elif '16.bvec' in fname:
                    shutil.copy2(fname, Diff_AP_bvec_fpath)
                elif 'DWI_PA' in fname and '20.nii.gz' in fname:
                    shutil.copy2(fname, Diff_PA_SBRef_fpath)
                elif 'DWI_PA' in fname and '21.nii.gz' in fname:
